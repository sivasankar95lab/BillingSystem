        ...
        saveCurrent() {
            ...
            if (existingIndex !== -1) {
                NotificationManager.confirm(
                    ...
                    () => {
                        // Update existing
                        const newRecord = { ...snapshot, savedAt: this.history[existingIndex].savedAt }; // Keep original savedAt for updates? Or update it? Let's update it to now.
                        // Actually, if we overwrite, we usually want to bump it to top or just update content.
                        // Let's update content and savedAt.
                        this.history[existingIndex] = snapshot;
                        this.saveHistory();
                        if (window.SyncManager) window.SyncManager.pushToCloud();
            ...
            if (sortedHistory.length === 0) {
                ...
                sortedHistory.forEach((record, index) => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td data-label="Actions">
                            <button class="btn btn-sm btn-primary me-1" onclick="InvoiceManager.load(${originalIndex})">
                                <i class="fas fa-box-open"></i> Load
                            </button>
                            <button class="btn btn-sm btn-outline-danger" onclick="InvoiceManager.delete(${originalIndex})">
                                <i class="fas fa-trash"></i>
                            </button>
                        </td>
                    `;
            ...
            try {
                ...
                keys.forEach(key => {
                    if (data.hasOwnProperty(key)) {
                        const val = data[key];
                        // If null, maybe remove? Or set as null string? 
                        // Usually local storage strings.
    ...
    const ModalDragger = {
        ...
        makeDraggable(modal, header, dialog) {
            ...
            const dragMouseDown = (e) => {
                ...
                // Get current relative position (defaults to 0 if not set)
                // We use style.left/.top which are explicit offsets from the 'centered' position
            ...
            // Reset position on close to avoid weirdness on reopen
